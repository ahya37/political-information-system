...
@section('content')
    <div class="container">
        <div class="page-content"> 
			  ...
			  <div class="card"> 
                <div class="card-body">
					<form id="exportPdf" class="mb-3" method="POST" action="{{route('admin-sip-rangking')}}">
					@csrf
					...
					<table class='table' id='datatable'>
						...
						<tbody>
							...
							<tr>
								<td >{{$no++}}</td>
								<td ><a href="{{route('admin-sip-dashboard-dapil', $item['id'])}}" target="_blank">{{$item['dapil']}}</a></td>
								<td class="text-center">{{$gf->decimalFormat($item['tps'])}}</td>
								<td class="text-center">{{$gf->decimalFormat($item['jml_all_anggota_dapil'])}}</td>
						...
						<tfoot id="datajmlsuara"> 
							<tr> 
								...
								<td id="jmlanggota"  class="text-center"><b>{{$gf->decimalFormat($jml_all_anggota_dapil)}}</b></td>  
