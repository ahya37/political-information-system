                            ...
                            <ul class="nav nav-tabs mb-4" id="myTab" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active" id="home-tab" data-toggle="tab" data-target="#home"
                                        type="button" role="tab" aria-controls="home" aria-selected="true">Per Kor
                                        TPS</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="profile-tab" data-toggle="tab" data-target="#profile"
                                        type="button" role="tab" aria-controls="profile" aria-selected="false">Keluarga
                                        Serumah</button>
                                </li>
                            </ul>
                                    ...
                                    <div class="card mb-2">
                                        <div class="card-body">
                                            <div class="col-md-12">
                                                <div class="row">
                                                    <div class="col-md-11">
                                                        <h5 class="card-title">{{ $no_head_familly++ }}. {{
                                                            $item['head_famlly_name'] }}</h5>
                                                    </div>
                                                    <div class="col-md-1">
                                                        <button class="btn btn-sm fa fa-trash text-danger"
                                                            onclick="onDeleteHeadFamilyGroup(this)"
                                                            data-name="{{ $item['head_famlly_name'] }}"
                                                            id="{{ $item['id'] }}"></button>
                                                    </div>
                                                </div>
                                            </div>
                                            <table id="familyGroup" class="table table-sm table-striped familyGroup"
                                                width="100%">
                                                <thead>
                                                    <tr>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{{ $row->name }}</td>
                                                        <td>{{ $row->address, 'DS.'.$row->village.',
                                                            KEC.'.$row->district }}</td>
                                                        <td>{{ $row->tps_number }}</td>
                                                        <td>{{ $row->telp }}</td>
                                                        <td>
                                                            <button type="button" class="btn btn-sm btn-danger"
                                                                onclick="onDeleteMemberFamilyGroup(this)"
                                                                data-name="{{ $row->name }}" id="{{ $row->id }}"><i
                            ...
                            <table id="anggotakortps" class="table table-sm table-striped mt-3" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr class="{{ $item->is_cover == 1 ? 'bg-success text-white' : '' }}">
                                        ...
                                        <td>{{ $item->nik }}</td>
                                        <td><button class="btn btn-sm btn-danger" data-name="{{ $item->name }}"
                                                data-whatever="{{ $item->id }}" data-toggle="modal"
                                                data-target="#exampleModal2"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin-sticker-save', $korte_idx) }}" method="POST"
    ...
    <div class="modal fade" id="exampleModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin-koordinatortpskorte-delete') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
